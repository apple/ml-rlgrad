...
class IMDB(TextGenPool):
    ...
    @classmethod
    def prepare(cls, split: str, seed: int, only_positive_samples: bool = False):
        ...
        for ix, (text, label) in enumerate(zip(dataset_split["text"], dataset_split["label"])):
            ...
            if (only_positive_samples and split == "train" and label == 1) or not only_positive_samples or split != "train":
                sample = Sample(id=f"{split}_{ix}",
...
class IMDBForSeq2Seq(TextGenPool):
    ...
    @classmethod
    def prepare(cls, split: str, positive_ratio: int = 1.0, seed: int = -1):
        dataset = load_dataset("imdb", ignore_verifications=True)
        train_val_dataset_split = dataset["train"].shuffle(seed) if seed >= 0 else dataset["train"].shuffle()
        if split in ["train", "val"]:
            train_ratio = 0.8
            train_index = int(len(train_val_dataset_split) * train_ratio)
            dataset_split = train_val_dataset_split[:train_index] if split == "train" else train_val_dataset_split[train_index:]
        else:
            # limit test to 5000
            dataset_split = dataset[split].shuffle(seed) if seed >= 0 else dataset[split].shuffle()
            dataset_split = dataset_split[:5000]
