...
class CausalLMActorCriticPolicy(LMActorCriticPolicy, ActorCriticWarmStartMixin):
    ...
    def _prepare_inputs_for_model(
        ...
        if self._apply_model_parallel and unwrap_model(model).is_parallelizable and torch.cuda.device_count() > 0:
    ...
    def forward_policy(
        ...
        next_token_logits = output.logits[:, -1, :] / self._temperature
...
class MaskedCausalLMActorCriticPolicy(
    ...
    def _build_model_heads(self, model_name: str):
        ...
        if torch.cuda.is_available():
            if unwrap_model(self._mask_model).is_parallelizable and self._apply_model_parallel and torch.cuda.device_count() > 0:
