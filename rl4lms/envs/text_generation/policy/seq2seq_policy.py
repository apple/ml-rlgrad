...
class Seq2SeqLMActorCriticPolicy(LMActorCriticPolicy, ActorCriticWarmStartMixin):
    ...
    def forward_policy(
        ...
        model_inputs["decoder_attention_mask"] = decoder_attn_mask
        outputs = self._policy_model(**model_inputs, return_dict=True)
        next_token_logits = outputs.logits[:, -1, :] / self._temperature
...
class MaskedSeq2SeqLMActorCriticPolicy(
    ...
    def _build_model_heads(self, model_name: str):
        ...
        if torch.cuda.is_available():
            if self._apply_model_parallel and unwrap_model(self._mask_model).is_parallelizable and torch.cuda.device_count() > 0:
