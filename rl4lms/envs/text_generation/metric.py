...
class MeteorMetricCommonGenPenalty(BaseMetric):
    def __init__(self) -> None:
        super().__init__()
        self._metric = load_metric("meteor")
    ...
    def compute(
        ...
        for prompt_text, generated_text in zip(prompt_texts, generated_texts):
            ...
            if (
                    concept_n_grams.lower() in generated_text.lower()
                    or prefix in generated_text.lower()
                    or "generate" in generated_text.lower()
                    or "sentence" in generated_text.lower()
            ):
                penalty_score_sum += -1

        score += penalty_score_sum / len(prompt_texts)
        metric_dict = {"lexical/meteor_common_gen_repeat_penalty": (None, score)}
        return metric_dict
...
class Perplexity(BaseMetric):
    ...
    def get_device(self, model: PreTrainedModel):
        try:
            return model.transformer.first_device if hasattr(model.transformer, "first_device") else torch.device("cpu")
        except:
            return model.device
...
class SacreBLEUMetric(BaseMetric):
    ...
    def compute(
        ...
        num_refs_to_gen_texts = defaultdict(list)
        num_refs_to_refs = defaultdict(list)
        for gen_text, sample_references in zip(generated_texts, reference_texts):
            num_refs = len(sample_references)
            num_refs_to_gen_texts[num_refs].append(gen_text)
            num_refs_to_refs[num_refs].append(sample_references)

        scores = []
        for num_refs in num_refs_to_gen_texts.keys():
            gen_texts_with_num_refs = num_refs_to_gen_texts[num_refs]
            references_for_texts = num_refs_to_refs[num_refs]
            metric_results = self._metric.compute(
                predictions=gen_texts_with_num_refs, references=references_for_texts, **self._args
            )
            scores.append(len(gen_texts_with_num_refs) * metric_results["score"] / 100)

        final_score = sum(scores) / len(generated_texts)
        metric_dict = {"lexical/sacrebleu": (None, final_score)}
        return metric_dict
...
class IntentAccuracyDailyDialog(BaseMetric):
    ...
    def compute(
        ...
        pred_labels = []
        with torch.no_grad():
            for start_batch_index in range(0, encoded.input_ids.shape[0], batch_size):
                end_batch_index = min(encoded.input_ids.shape[0], start_batch_index + batch_size)

                outputs = self._model(
                    input_ids=encoded.input_ids[start_batch_index:end_batch_index].to(self._device),
                    attention_mask=encoded.attention_mask[start_batch_index:end_batch_index].to(self._device),
                )
                pred_labels.extend(torch.argmax(outputs.logits, dim=1).tolist())
