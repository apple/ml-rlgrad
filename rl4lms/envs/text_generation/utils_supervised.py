...
def generate(model: PreTrainedModel,
    ...
    if model.config.is_encoder_decoder:
        # seq2seq LM
        first_device = model.encoder.first_device if hasattr(model.encoder, "first_device") else torch.device("cpu")
    else:
        # causal LM
        first_device = model.transformer.first_device if hasattr(model.transformer, "first_device") else torch.device("cpu")
    ...
    if return_probs:
        probs_of_all_tokens = torch.softmax(torch.stack(gen_output["scores"], dim=1), dim=-1)
        probs_gen_tokens = torch.gather(probs_of_all_tokens, dim=-1, index=gen_tokens.unsqueeze(dim=-1)).squeeze(dim=-1)
        return gen_texts, probs_gen_tokens
...
def tokenize_causal(item, tokenizer):
    outputs = tokenizer(item["content"], truncation=True)
    return {"input_ids": outputs["input_ids"]}


def tokenize_seq2seq(item, tokenizer):
    model_inputs = tokenizer(item["input_text"], truncation=True)
...
def generate_on_samples(model: PreTrainedModel,
    ...
    post_processing_fn = generation_kwargs.pop("post_processing_fn", None)
    if post_processing_fn is not None:
        post_processing_fn = PostProcessorRegistry.get(post_processing_fn["id"])
    ...
    for batch in tqdm(list(get_batch(samples, batch_size)), desc="Predicting"):
        if return_probs:
            batch_generated_texts, probs = generate_text(model, tokenizer, batch, max_prompt_length, generation_kwargs,
                                                         return_probs=return_probs)
        ...
        if post_processing_fn is not None:
            batch_generated_texts = [post_processing_fn(text) for text in batch_generated_texts]
    ...
    if return_probs:
        return all_prompt_texts, all_generated_texts, all_ref_texts, all_meta_infos, all_probs
