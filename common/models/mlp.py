...
class MultiLayerPerceptron(nn.Module):
    ...
    def gaussian_init_parameters(self, std: float = 1):
        """
        Initializes weight matrices using a zero-mean Gaussian with the specified standard deviation and biases to zero.
        """
        for child_module in self.hidden_layers_sequential.children():
            if isinstance(child_module, nn.Linear):
                torch.nn.init.normal_(child_module.weight, std=std)
                if hasattr(child_module, "bias"):
                    torch.nn.init.zeros_(child_module.bias)
