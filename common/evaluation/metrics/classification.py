...
class CrossEntropyLossOverProbabilities(AveragedMetric):
    """
    Cross entropy loss metric. Receives as input the probabilities of the prediction and the true labels.
    """
    ...
    def _calc_metric(self, y_pred_probs, y):
        ...
        relevant_indices = torch.nonzero(y >= 0, as_tuple=False).squeeze()  # Ignore negative labels (missing labels)
        y = y[relevant_indices]
        y_pred_probs = y_pred_probs[relevant_indices]
        if len(y) == 0:
            return 0, 0

        loss = F.nll_loss(torch.log(y_pred_probs), y)
        return loss.item(), len(y)
...
class ThresholdBinaryClassificationAccuracy(AveragedMetric):
    ...
    def __init__(self, first_label_value: float, second_label_value: float, threshold_weight: float = 0.5):
        ...
        :param threshold_weight: if prediction is at least negative_label_value + (positive_label_value - negative_label_value) * threshold_weight
        then it is considered as positive, otherwise as negative.
        """
        super().__init__()
